...
func _physics_process(delta: float) -> void:
	...
	if !_on_cooldown && _player_in_portal:
	...
	# Camera flipping
	if _portal_core.has_overlapping_areas():
...
func flip_cameras() -> void:
	...
	if _main_cam_in_overworld:
		_remote_portal_cam.set_remote_node(_portal_viewport_cam_parent.get_child(0).get_path())
		_main_cam_in_overworld = false
...
func _spawn_frame() -> void:
	...
	# adjust portal core area collider
	var core_col = _portal_core.find_child("PortalCol")
	core_col.shape.set_size(Vector3(width, height, _thickness / 3.0))
